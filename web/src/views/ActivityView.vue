...
<template>
  <section class="glass-panel page-panel">
    ...
    <div class="activity-summary" aria-label="记录分类统计">
      <button :class="{ active: filter === 'all' }" @click="filter = 'all'"><span class="summary-mark all" /><strong>{{ total }}</strong><span>全部记录</span></button>
  ...
  <el-dialog v-model="detailVisible" title="单次对话 / 决策详情" width="min(720px, calc(100vw - 28px))">
    <div v-if="detail" class="event-detail">
      ...
      <details v-if="detail.model_usages.length" class="detail-section diagnostic-section">
        <summary class="detail-label">模型与工具诊断 <small>{{ detail.model_usages.length }} 次调用，默认折叠</small></summary>
        <div class="retrieval-list"><div v-for="item in detail.model_usages" :key="`${item.trace_id}-${item.iteration}`" class="retrieval-row"><div><b>第 {{ item.iteration }} 次调用</b><small>trace {{ item.trace_id }} · {{ item.input_tokens }} 输入 token（缓存 {{ item.cached_tokens }} / 未命中 {{ item.cache_miss_tokens }}）· {{ item.output_tokens }} 输出 token · {{ item.duration_ms }}ms · {{ item.tools.join(', ') || '无工具' }}</small><small>上下文 {{ item.prompt_shape.static_bytes }}B 静态 · {{ item.prompt_shape.session_bytes }}B Session · {{ item.prompt_shape.current_turn_bytes }}B 当前轮 · {{ item.prompt_shape.memory_bytes }}B 记忆 · {{ item.prompt_shape.tool_schema_bytes }}B 工具 Schema</small><small>动作 {{ item.final_action || '—' }} · {{ item.sent ? '已发送' : `未发送${item.drop_reason ? ` · ${item.drop_reason}` : ''}` }}</small><div v-if="item.tool_calls.length" class="tool-call-list"><small v-for="toolCall in item.tool_calls" :key="`${toolCall.name}-${toolCall.duration_ms}-${toolCall.arguments}`">工具 {{ toolCall.name }} · 参数 {{ toolCall.arguments }} · 结果 {{ toolCall.result }} · {{ toolCall.duration_ms }}ms<template v-if="toolCall.error"> · {{ toolCall.error }}</template></small></div><small v-if="item.error" class="task-error">{{ item.error }}</small></div><el-tag v-if="item.error" type="danger" effect="plain">失败</el-tag><el-tag v-else-if="item.sent" type="success" effect="plain">已发送</el-tag></div></div>
      </details>
    </div>
  </el-dialog>
</template>
