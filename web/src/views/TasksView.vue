...
<template>
  <section class="glass-panel page-panel">
    ...
    <div class="task-summary"><span>共 {{ total }} 条 · 本页 {{ rows.length }} 条</span><span>本页待处理 {{ counts.pending || 0 }}</span><span>本页重试 {{ counts.retry || 0 }}</span><span class="task-summary-danger">本页死信 {{ counts.dead_letter || 0 }}</span></div>
