...
curator turn、媒体感知和 memory 向量同步已完成迁移：分别以 `snapshot_id`、`event_id`、`memory_id` 为幂等键进入 outbox，由注册 handler 执行 curator/review、perception 与 Qdrant 写入；meme 和 learning 任务按同一模式逐步迁移。
