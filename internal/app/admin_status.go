...
func (h *adminHandler) buildSystemStatus(snapshot adminSnapshot) *SystemStatus {
	now := time.Now()
...
func (h *adminHandler) buildIncrementalUpdate(snapshot adminSnapshot, since int64) *IncrementalUpdate {
	now := time.Now()
