...
func TestManagerEnqueuesProactiveCandidateThroughActor(t *testing.T) {
	...
	if selected.CandidateID != candidate.CandidateID || selected.DeliveryTarget != "group" || selected.Status != presencedomain.CandidateAccepted {
		t.Fatalf("unexpected candidate defaults: %+v", selected)
	}
	loaded, err := store.LoadWorkingMemory(context.Background(), 9)
	if err != nil || len(loaded.Candidates) != 1 || loaded.Candidates[0].Status != presencedomain.CandidateAccepted {
...
func TestManagerPrunesIdleActorsButKeepsLiveCandidates(t *testing.T) {
	...
	idleRecord.Origin = presencedomain.OriginOutbound
	...
	due := time.Now()
	if err := manager.EnqueueCandidate(context.Background(), 22, presencedomain.ThoughtCandidate{
...
func eventRecord(id string, groupID, userID int64, text string, at time.Time) presencedomain.EventRecord {
